...
path = "D:\\News\\"
pathCheck = "D:\\News"
loggerFileName = "D:\\Advanced Python Project\\logFile.log"
            ...
            for row in csv_reader:
                ...
                if numWords == 140:  # dont summarize
                    ...
                    if summarized_content != '':
                        ...
                        csv_writer.writerow([row[0], original_content])
...
def DRIVERCODE():
    ...
    if not os.path.exists(pathCheck):
        ...
        logger.info(f"Folder 'News' created in path: {path}")
...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', filename=loggerFileName, filemode="w", force=True)
